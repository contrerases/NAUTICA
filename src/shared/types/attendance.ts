...
export interface AttendanceRecord {
  ...
  // HECHOS del turno (minutos). NULL/0 mientras está OPEN.
  ...
  // Snapshot del HORARIO vigente al marcar (para recomputar minutos al editar).
  ...
  // ── CALCULADOS al leer (no son columnas; los llena el servicio al servir) ──
  // El dinero se resuelve con el valor hora vigente por fecha y el modelo del turno.
  hourly_rate: number; // valor hora vigente para la fecha del turno
  pay_model: PayModel; // modelo vigente para la fecha del turno
  base_payment: number; // 0 en sueldo fijo (la base va en el sueldo)
